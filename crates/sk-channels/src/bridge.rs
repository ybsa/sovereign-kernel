...
async fn dispatch_message(
    ...
    router: &Arc<AgentRouter>,
    adapter: &dyn ChannelAdapter,
    _rate_limiter: &ChannelRateLimiter,
) {
    ...
    let text = match &message.content {
        ...
        ChannelContent::Command { name, args } => {
            ...
            send_response(adapter, &message.sender, result, thread_id).await;
            return;
        }
        _ => {
            send_response(
    ...
    if router.has_broadcast(&message.sender.platform_id) {
        ...
        if !targets.is_empty() {
            ...
            // Broadcast to all resolved targets
            ...
            send_response(adapter, &message.sender, combined, thread_id).await;
            return;
        }
    }
    ...
    if let Some(reply) = handle.check_auto_reply(agent_id, &text).await {
        send_response(adapter, &message.sender, reply, thread_id).await;
        handle
    ...
    match handle.send_message(agent_id, &text).await {
        Ok(response) => {
            send_response(adapter, &message.sender, response, thread_id).await;
            handle
                .record_delivery(agent_id, ct_str, &message.sender.platform_id, true, None)
                .await;
        }
        Err(e) => {
            ...
            send_response(adapter, &message.sender, err_msg.clone(), thread_id).await;
            handle
                .record_delivery(
                    agent_id,
                    ct_str,

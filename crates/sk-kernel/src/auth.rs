...
use sk_types::config::UserConfig;
...
pub struct AuthManager {
    /// Known users by their kernel user ID.
...
impl AuthManager {
    ...
    ///
    /// Returns the kernel UserId if a matching channel binding exists,
    ...
    pub fn authorize(&self, user_id: UserId, action: &Action) -> SovereignResult<()> {
        let identity = self
            .users
            .get(&user_id)
            .ok_or_else(|| SovereignError::AuthDenied("Unknown user".to_string()))?;

        let required = action.required_role();
        if identity.role >= required {
            Ok(())
        } else {
            Err(SovereignError::AuthDenied(format!(

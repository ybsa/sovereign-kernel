...
use dashmap::DashMap;
use sk_types::agent::{AgentEntry, AgentId, AgentMode, AgentState};
use sk_types::error::{SovereignError, SovereignResult};
...
impl AgentRegistry {
    ...
    pub fn register(&self, entry: AgentEntry) -> SovereignResult<()> {
        if self.name_index.contains_key(&entry.name) {
            return Err(SovereignError::AgentAlreadyExists(entry.name.clone()));
        }
        let id = entry.id;
    ...
    /// Update agent state.
    pub fn set_state(&self, id: AgentId, state: AgentState) -> SovereignResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SovereignError::AgentNotFound(id.to_string()))?;
        entry.state = state;
    ...
    pub fn set_mode(&self, id: AgentId, mode: AgentMode) -> SovereignResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SovereignError::AgentNotFound(id.to_string()))?;
    ...
    pub fn remove(&self, id: AgentId) -> SovereignResult<AgentEntry> {
        let (_, entry) = self
            .agents
            .remove(&id)
            .ok_or_else(|| SovereignError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_session_id(
        &self,
        id: AgentId,
        new_session_id: sk_types::agent::SessionId,
    ) -> SovereignResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_identity(
        &self,
        id: AgentId,
        identity: sk_types::agent::AgentIdentity,
    ) -> SovereignResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_model(&self, id: AgentId, new_model: String) -> SovereignResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SovereignError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_skills(&self, id: AgentId, skills: Vec<String>) -> SovereignResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SovereignError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_mcp_servers(&self, id: AgentId, servers: Vec<String>) -> SovereignResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SovereignError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_system_prompt(&self, id: AgentId, new_prompt: String) -> SovereignResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SovereignError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_name(&self, id: AgentId, new_name: String) -> SovereignResult<()> {
        if self.name_index.contains_key(&new_name) {
            return Err(SovereignError::AgentAlreadyExists(new_name));
        }
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_description(&self, id: AgentId, new_desc: String) -> SovereignResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SovereignError::AgentNotFound(id.to_string()))?;
    ...
    pub fn mark_onboarding_complete(&self, id: AgentId) -> SovereignResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SovereignError::AgentNotFound(id.to_string()))?;

...
pub fn app_installer_tool() -> ToolDefinition {
    ToolDefinition {
        ...
        description: "Install applications on the host system using winget. Requires Unrestricted mode.".into(),
...
pub fn handle_app_installer(package_id: &str) -> Result<String, sk_types::SovereignError> {
    let output = Command::new("winget")
        ...
        .output()
        .map_err(|e| sk_types::SovereignError::ToolExecutionError(format!("Failed to execute winget: {}", e)))?;
    ...
    if output.status.success() {
        ...
        Err(sk_types::SovereignError::ToolExecutionError(format!("Winget failed: {}\n{}", stdout, stderr)))
    }
}

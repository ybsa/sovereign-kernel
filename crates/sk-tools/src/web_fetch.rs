...
pub async fn handle_web_fetch(url: &str) -> Result<String, sk_types::SovereignError> {
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|e| sk_types::SovereignError::ToolExecutionError(format!("Failed to build HTTP client: {}", e)))?;

    let response = client.get(url)
        .send()
        .await
        .map_err(|e| sk_types::SovereignError::ToolExecutionError(format!("HTTP request failed: {}", e)))?;

    let status = response.status();
    let text = response.text().await
        .map_err(|e| sk_types::SovereignError::ToolExecutionError(format!("Failed to read response body: {}", e)))?;
...
fn clean_html(html: &str) -> String {
    ...
    let title = if let Some(caps) = re_title.captures(&html_clean) {
        caps.get(1).map(|m| m.as_str().trim().to_string()).unwrap_or_else(|| "No Title".to_string())
    } else {
        "No Title".to_string()
    };

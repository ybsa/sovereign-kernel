...
impl KnowledgeStore {
    ...
    pub fn find_entities(&self, agent_id: AgentId, pattern: &str) -> SovereignResult<Vec<Entity>> {
        ...
        let rows = stmt.query_map(rusqlite::params![agent_id.to_string(), pattern], |row| {
        ...
        for row in rows {
            let (id, name, entity_type, props_str) = row.map_err(|e| SovereignError::MemoryError(e.to_string()))?;
            let properties: serde_json::Value = serde_json::from_str(&props_str).unwrap_or_default();
            entities.push(Entity { id, agent_id, name, entity_type, properties });
        }
        Ok(entities)
    }
